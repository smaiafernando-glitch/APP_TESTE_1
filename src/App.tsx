...
const Onboarding = ({ onComplete }) => {
  ...
  const [formData, setFormData] = useState({ 
    ...
    phone: '', 
    name: '', 
    patientToWatch: '' // Only for caregiver
  });
  ...
  if (step === 'welcome') return (
    <div className="flex flex-col h-screen px-8 pb-12 pt-24 animate-in fade-in duration-700">
      ...
      <Button fullWidth onClick={() => setStep('role')}>Começar</Button>
    </div>
  );
    ...
    <div className="flex flex-col h-screen px-8 pt-16 animate-in slide-in-from-right-8 duration-300">
      ...
      <div className="space-y-4">
        <button 
          ...
          className={`w-full p-6 rounded-[28px] border-2 transition-all flex items-center gap-4 ${formData.role === 'patient' ? 'border-blue-500 bg-blue-50 shadow-md' : 'border-white bg-white shadow-sm'}`}
        >
        ...
        <button 
          ...
          className={`w-full p-6 rounded-[28px] border-2 transition-all flex items-center gap-4 ${formData.role === 'caregiver' ? 'border-indigo-500 bg-indigo-50 shadow-md' : 'border-white bg-white shadow-sm'}`}
        >
      ...
      <div className="mt-auto pb-12">
        <Button fullWidth disabled={!formData.role} onClick={() => nextStep('phone')}>Continuar</Button>
      </div>
    </div>
  );
  ...
  if (step === 'phone') return (
    <div className="flex flex-col h-screen px-8 pt-16 animate-in slide-in-from-right-8 duration-300">
      <button onClick={() => setStep('role')} className="mb-8 w-10 h-10 flex items-center justify-center bg-white rounded-xl shadow-sm"><ChevronLeft /></button>
  ...
  if (step === 'profile') return (
    <div className="flex flex-col h-screen px-8 pt-16 animate-in slide-in-from-right-8 duration-300 overflow-y-auto pb-12">
      ...
      {formData.role === 'caregiver' && (
        <div className="mt-4 animate-in fade-in duration-500">
          ...
          <Input label="Nome do Paciente" placeholder="Ex: Maria Silveira" icon={Heart} value={formData.patientToWatch} onChange={(e) => setFormData({...formData, patientToWatch: e.target.value})} />
        </div>
      )}

      <div className="mt-8"><Button fullWidth loading={loading} disabled={!formData.name} onClick={() => nextStep('success')}>Finalizar Cadastro</Button></div>
    </div>
  );
  ...
  if (step === 'success') return (
    <div className="flex flex-col h-screen px-8 items-center justify-center text-center animate-in zoom-in-95 duration-500">
      ...
      <p className="text-gray-500 text-lg mb-10">O VIVERCOM está pronto para <br/> {formData.role === 'patient' ? 'cuidar de você' : `acompanhar ${formData.patientToWatch}`}.</p>
      <Button fullWidth onClick={() => onComplete(formData)}>Ir para o Início</Button>
    </div>
  );
};
...
const DashboardModule = ({ user }) => {
  ...
  return (
    <div className="p-6 pb-24 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      {/* Seção de Disposição - Apenas se for Paciente */}
      {!isCaregiver && (
        <div className="mb-8">
      ...
      {/* Se for Rede de Apoio, mostra o status do paciente */}
      {isCaregiver && (
...
// ... Resto dos componentes AgendaModule, HistoryModule, ProfileModule mantidos iguais do artefato anterior ...
const AgendaModule = () => (
...
const HistoryModule = () => (
    <div className="p-6 pb-24 animate-in fade-in duration-500 bg-[#F2F2F7]">
      <div className="flex justify-between items-center mb-6"><h1 className="text-3xl font-bold">Saúde</h1><button className="bg-white p-2.5 rounded-xl shadow-sm border border-gray-100"><Filter size={20} className="text-gray-500" /></button></div>
      <div className="grid grid-cols-2 gap-4 mb-8">
        {MOCK_VITALS.map((vital) => (
          <Card key={vital.id} className="p-4 flex flex-col justify-between h-32 relative overflow-hidden group">
            <div className="flex justify-between items-start"><div className={`p-2 rounded-xl ${vital.bg} ${vital.color}`}><vital.icon size={20} /></div><ArrowUpRight size={16} className="text-gray-300 group-hover:text-blue-500 transition-colors" /></div>
            <div><div className="flex items-baseline gap-1"><span className="text-2xl font-black text-gray-900">{vital.value}</span><span className="text-xs font-bold text-gray-400 uppercase">{vital.unit}</span></div><p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">{vital.label}</p></div>
          </Card>
        ))}
      </div>
      <div className="space-y-4 relative before:content-[''] before:absolute before:left-6 before:top-2 before:bottom-2 before:w-[2px] before:bg-gray-200">
        {MOCK_HISTORY.map((item) => (
          <div key={item.id} className="relative pl-14">
            <div className="absolute left-3 top-2 w-6 h-6 rounded-full border-4 border-[#F2F2F7] z-10 bg-blue-500"></div>
            <Card className="p-4 hover:shadow-md transition-shadow cursor-pointer">
              <div className="flex justify-between items-start">
                <div><span className="text-[9px] font-black uppercase tracking-wider text-blue-600 bg-blue-50 px-2 py-0.5 rounded-md mb-1 inline-block">{item.category}</span><h4 className="font-bold text-gray-800 text-base">{item.title}</h4><p className="text-xs text-gray-400 font-medium">{item.date}</p></div>
    ...
    <div className="p-6 pb-24 animate-in fade-in duration-500">
      ...
      <div className="space-y-3">
        {['Dados Pessoais', 'Notificações', 'Segurança', 'Rede de Apoio'].map((item, idx) => (
          <button key={idx} className="w-full bg-white p-5 rounded-[24px] flex items-center gap-4 shadow-sm active:scale-[0.98] transition-all">
            <div className="w-10 h-10 rounded-xl bg-gray-50 flex items-center justify-center text-gray-400"><Settings size={20} /></div>
